...
def buildLRArray(
    ...
    def _buildLRArray(L, R, _L, _R, off, n, descBar: tqdm):
    ...
    # Create description bars for debugging
    descBars = []
    for i in range(ncores):
        if i != ncores - 1:
            descBar = tqdm(desc=f'#{i}', total=nPerCore)
        else:
            descBar = tqdm(desc=f'#{i}', total=nLstCore)
        descBars.append(descBar)
...
def sortLRArray(sharedMem, n: int, p: int, ncores):
    ...
    def _partition(view: memoryview, lo: int, hi: int, descBar: tqdm):
        i     = lo
    ...
    def _quicksort(sharedMem, lo: int, hi: int, descBar: tqdm):
        ...
        while sortQueue:
            ...
            # Get partition index
            pi = _partition(view, lo, hi, descBar)
    ...
    for i in range(ncores):
        if i != ncores - 1:
            futures.append(Process(
                            target=_quicksort, 
                            args=(sharedMem, i*nPerCore, (i+1)*nPerCore-1, descBars[i])
                          ))
        else:
            futures.append(Process(
                            target=_quicksort, 
                            args=(sharedMem, i*nPerCore, n-1, descBars[i])
                          ))
...
def searchLRArray(L, R, n: int, p: int, ncores):
    ...
    def _searchLRArray(L, R, lL, hL, lR, hR, retQueue, descBars: list[tqdm]):
        ...
        while True:
            ...
            if memcmp(viewL, viewR, indexSize, fieldSize) == 0:
                descBars[0].update(descBars[0].total)
                descBars[1].update(descBars[1].total)
                retQueue.put([
    ...
    for iL in range(ncores):
        ...
        for iR in range(ncores):
            descBars.append(tqdm(desc=f'#L{iL}-R{iR}', total=(nPerCore if iL != ncores - 1 else nLstCore)))
            descBars.append(tqdm(desc=f'#R{iR}-L{iL}', total=(nPerCore if iR != ncores - 1 else nLstCore)))
...
def discrete_log_elliptic_curve_Fp(X, Y, ncores=4):
    with SharedMemoryManager() as memManager:
        ...
        #################### PART 1: GENERATE L&R ########################
        # Initialize arrays
        print(f'[*] Memory consumption (for baby-giant arrays): {2*fieldSize*n} bytes')
        ...
        print(f'[*] Creating baby and giant arrays for collision attack :)')
        buildLRArray(
            L, R, 
            X, Y, 
            n, p,
        ...
        print(f'[*] Sorting L memory...')
        sortLRArray(L, n, p, ncores)
    
        print(f'[*] Sorting R memory...')
